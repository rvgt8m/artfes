...
写真のジャンル</td><td class="tdc">
<?
$rr=check_string($RV[photoGenre],1,2,1,'photoGenre');
echo mb_convert_encoding($rr[1],'utf-8','EUC-JP').$rr[2];
if($rr[1]!=''){
	$ac++;
}
?>
<?
	$valAr=explode('_',$photoGenreAr[$RV[photoGenre]]);
echo
	$currName=$valAr[($lang-1)];
?>
...
サービスタイム</td><td class="tdc">
<?
$rr=check_string($RV[timeOfMenu],1,2,1,'timeOfMenu');
echo mb_convert_encoding($rr[1],'utf-8','EUC-JP').$rr[2];
if($rr[1]!=''){
#	$ac++;
}
?>
<?
	$valAr=explode('_',$timeOfMenuAr[$RV[timeOfMenu]]);
echo
	$currName=$valAr[($lang-1)];
?>
...
日本語：<?=htmlspecialchars($RV[title_j], ENT_QUOTES)?>
<?
$rr=check_string($RV[title_j],0,100,1,'title_j');
echo mb_convert_encoding($rr[1],'utf-8','EUC-JP');
if($rr[1]!=''){
#	$ac++;
}
?>
...
<input type="hidden" name="title_j" value="<?=htmlspecialchars($RV[title_j], ENT_QUOTES)?>">

<br />
英語：<?=htmlspecialchars($RV[title_e], ENT_QUOTES)?>
<?
$rr=check_string($RV[title_e],3,100,1,'title_e');
#echo $rr[1];
if($rr[1]!=''){
#	$ac++;
}
?>
<input type="hidden" name="title_e" value="<?=htmlspecialchars($RV[title_e], ENT_QUOTES)?>">

<br />
<input type=hidden name="is_main" value="<?=$RV[is_main]?>" /><?if($RV[is_main]==1){echo " この画像をメイン画像にする";}?>
...
<?
	...
	for($r=0;$r<$TSt;$r++){
		if($RV["type_id".$r]==$TSl[$r][type_id]){
			array_push($votedTxtAr,@$TSl[$r]["type_name".$LT]);

?>

<input type=hidden name="type_id<?=$r?>" value="<?=$TSl[$r][type_id]?>">
...
<?
	...
	for($r=1;$r<count($vegeTypeAr);$r++){
		...
		$currName=$valAr[($lang-1)];
		if($RV["vegeType".$r]!=''){
			array_push($votedTxtAr,$currName);
			$vc++;
?>
<input type=hidden name="vegeType<?=$r?>" value="<?=$r?>" />
...
<?
	...
	if($vc==0 && $RV[vegeType0]==''){
?>
<span class=alt>未選択です。</span>
<?
	}
...
<br />
<br />
▼その他に含まれる動物性、動物由来のもの<br />

日本語：<?=htmlspecialchars($RV[vegeLevelTxt_j], ENT_QUOTES)?>

<?
		$rr=check_string($RV[vegeLevelTxt_j],0,100,1,'vegeLevelTxt_j');
		echo $rr[1];
		if($rr[1]!=''){
			$ac++;
		}
...
英語：<?=htmlspecialchars($RV[vegeLevelTxt_e], ENT_QUOTES)?>

<?
		$rr=check_string($RV[vegeLevelTxt_e],3,100,1,'vegeLevelTxt_e');
		echo $rr[1];
		if($rr[1]!=''){
		#	$ac++;
		}
...
?>
<input type="hidden" name="vegeLevelTxt_j" value="<?=htmlspecialchars($RV[vegeLevelTxt_j], ENT_QUOTES)?>">
<input type="hidden" name="vegeLevelTxt_e" value="<?=htmlspecialchars($RV[vegeLevelTxt_e], ENT_QUOTES)?>">

<br /><br />
<span class=alt>※</span>微量でも、素材、料理油、調味料など料理に含まれる動物性をご選択・ご入力下さい。<br />
...
<?
	if($RV[note_j]!=''){
		$rr=check_string($RV[note_j],0,$limitComment,1,'note_j');
//		echo @mb_convert_encoding($rr[1],'utf-8',auto);
		if($rr[1]!=''){
			$ac++;
		}
	}
...
<?
	if($RV[note_e]!=''){
		$rr=check_string($RV[note_e],3,$limitComment,1,'note_e');
//		echo @mb_convert_encoding($rr[1],'utf-8',auto);
		if($rr[1]!=''){
#			$ac++;
		}
...
<input type="hidden" name="note_j" value="<?=htmlspecialchars($RV[note_j], ENT_QUOTES)?>">
<input type="hidden" name="note_e" value="<?=htmlspecialchars($RV[note_e], ENT_QUOTES)?>">
...
<?
	if($RV[price]!=''){
		$rr=check_string($RV[price],1,$limitComment,1,'price');
		echo @mb_convert_encoding($rr[1],'utf-8','SJIS');
		if($rr[1]!=''){
			$ac++;
		}
?>

<?=number_format($RV['price'])?> 円
<input type="hidden" name="price" value="<?=$RV['price']?>">
...
<?=$RV['y']?>年<?=$RV['m']?>月<?=$RV['d']?>日


<input type="hidden" name="y" value="<?=$RV['y']?>">
<input type="hidden" name="m" value="<?=$RV['m']?>">
<input type="hidden" name="d" value="<?=$RV['d']?>">
...
<input type="hidden" name="cameraman_id" value="<?=$RV[cameraman_id]?>">
...
<input type="hidden" name="is_del" value="<?=$RV[is_del]?>">
...
<input type="button"  value="戻る" onClick="document.f.mode.value='c';document.f.submit();" class="btnB">
<?
/**/
$WHERE=" WHERE d_mem_id ='".$d_mem_id."'";
list($WSe,$WSl,$WSt)=select_seek ($CON,'w_works','','','',$WHERE,'');
if($WSt>=$limit && $RV[works_id]==''){
	echo " <span class=alt>作品数制限：<b>".$limit." </b>に達しています</span>";
	$ac++;
}
$cc='登録';
$dd='';
...
?> <?}?>
<input type="submit"  value="<?=$cc?>" class="btnB<?=$dd?>">
</td></tr>
